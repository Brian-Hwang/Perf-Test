...
def generate_fio_config(num_jobs, file_path_options, converged):
    base_config = '''[global]
bs=128k
iodepth=32
direct=1
...
'''
    ...
    job_configs = []
    job_count = 0
    for i in range(num_jobs):  # Outer loop for number of jobs
        for file_path_option in file_path_options:  # Inner loop for each file path
            job_config = f'''[job{job_count+1}]
rw=randread
filename=/mnt/{file_path_option}/1g_file_{i}'''
            job_config += f'\ncpus_allowed={job_count+5}'  # Set cpus_allowed for all jobs
...
def run_fio(num_jobs, file_path_options, converged):
    config = generate_fio_config(num_jobs, file_path_options, converged)
...
def main():
    ...
    all_file_paths = ['first', 'second', 'third']
    ...
    if args.converged:
        ...
        for nj in range(1, args.jobs + 1):
            ...
            output = run_fio(nj, file_paths, True)
        ...
        for file_path_option in file_paths:
            ...
            for nj in range(1, args.jobs + 1):
                ...
                output = run_fio(nj, [file_path_option], False)
