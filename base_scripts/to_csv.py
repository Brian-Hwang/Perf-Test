...
with open(txt_file_path, 'r') as file:
    for line in file:
        ...
        if match:
            ...
            bandwidth = float(match.group(3)) * 1000  # Convert Gbits/sec to Mbits/sec
...
with open(csv_file_path, 'w', newline='') as file:
    ...
    writer.writerow(['Iteration'] + ['P{}'.format(p) for p in sorted_parallels])  # Header row
    for iteration, bandwidths in sorted(data_dict.items()):
        writer.writerow([iteration] + [bandwidths.get(p, '') for p in sorted_parallels])
